...
class Estimator():
    ...
    def _build_model(self):
        ...
        print("shape: of prediction {} and losses shape: {} ".format(self.predictions.get_shape(),self.losses.get_shape()))
        max_predict = tf.reduce_max(self.predictions)

        # Summaries for Tensorboard
        tf.summary.scalar("loss", self.loss),
        tf.summary.histogram("loss_hist", self.losses),
        tf.summary.histogram("q_values_hist", self.predictions),
        tf.summary.scalar("max_q_value", max_predict)
...
def check_file(path,scope):
    ...
    file_to_restore = [i.rstrip('.meta') for i in files if i.endswith('meta') and i.startswith(i.split(".")[0]+".data")] #making sure data file is there too
